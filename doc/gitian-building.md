...
*Setup instructions for a gitian build of Smc-coin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Smc-coin
...
- [Building Smc-coin](#building-smc-coin)
...
Clone the git repositories for smc-coin and gitian and then checkout the smc-coin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/kobrin/smc-coin.git
cd smc-coin
...
Follow the instructions in [doc/release-process.md](release-process.md) in the smc-coin repository
...
To build Smc-coin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the smc-coin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/smc-coin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/kobrin/smc-coin
...
For example:
```bash
URL=https://github.com/crowning-/smc-coin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit smc-coin=${COMMIT} --url smc-coin=${URL} ../smc-coin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit smc-coin=${COMMIT} --url smc-coin=${URL} ../smc-coin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit smc-coin=${COMMIT} --url smc-coin=${URL} ../smc-coin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/smc-coin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/smc-coin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/smc-coin-build.assert
```
...
[smc-coin/gitian.sigs](https://github.com/darknetpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
