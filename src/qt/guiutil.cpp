...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Smc-coin address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no Smc-coin: URI
    if(!uri.isValid() || uri.scheme() != QString("smc-coin"))
        return false;
...
{
    // Convert smc-coin:// to smc-coin:
    //
    //    Cannot handle this later, because darkent:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("smc-coin://", Qt::CaseInsensitive))
    {
        uri.replace(0, 12, "smc-coin:");
    }
...
{
    QString ret = QString("smc-coin:%1").arg(info.address);
    int paramCount = 0;
...
void openConfigfile()
{
    ...
    /* Open smc-coin.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Smc-coin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Smc-coin.lnk
...
{
    return GetAutostartDir() / "smc-coin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a smc-coin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Smc-coin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the smc-coin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add smc-coin app to startup item list
