...
Value getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total smc-coin balance of the wallet\n"
            "  \"obfuscate_balance\": xxxxxx, (numeric) the anonymized smc-coin balance of the wallet\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in smc-coin/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in smc-coin/kb\n"
    ...
    if (pwalletMain) {
        ...
        if(!fLiteMode)
            obj.push_back(Pair("obfuscate_balance",       ValueFromAmount(pwalletMain->GetAnonymizedBalance())));
    }
#endif
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nReturn information about the given smc-coin address.\n"
            "\nArguments:\n"
            "1. \"darknetaddress\"     (string, required) The smc-coin address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"darknetaddress\", (string) The smc-coin address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Smc-coin address and we have full public key:
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are smc-coin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) smc-coin address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"darknetaddress\"  (string, required) The smc-coin address to use for the signature.\n"
